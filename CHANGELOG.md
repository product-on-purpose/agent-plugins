...
- Re-pinned `agent-skills-toolkit` from **`v1.17.1`** (`4ba1ae6`) to **`v1.18.0`** (`e7e3dfc`).
  `strict: true` preserved; no other member entry moves.
- Opened automatically by `repin-watch`, which noticed that product-on-purpose/agent-skills-toolkit had published v1.18.0 while this registry was still pinned to v1.17.1. The re-pin checklist in `CONTRIBUTING.md` Section 7 is in the pull request body and is the maintainer's to complete; this automation does the noticing, not the deciding.
