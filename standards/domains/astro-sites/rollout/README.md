...
| [`writing-style-catalog.md`](writing-style-catalog.md) | writing-style-catalog | one merge away | P1 confirm A-6, merge the Pattern S branch |
...
1. **writing-style-catalog** first if you want the family to reach 4/4 Pattern S on `main` fastest (it is one confirm-and-merge).
2. **thinking-framework-skills** and **agent-skills-toolkit** next (small, mechanical).
...
Three of the four repos already run Astro Starlight (pm-skills, thinking-framework-skills, agent-skills-toolkit are Pattern S on `main`); writing-style-catalog has it staged on a branch. So each packet's real job is **conformance**, not greenfield install: close the gaps to the standard and lock them in. Each kickoff prompt is worded to match that repo's actual starting point.
