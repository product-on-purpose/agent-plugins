...
| `./styles/accent.css` | static CSS asset | The brand layer: the `--sl-color-accent` triplet (`#5C7CFA` with low/high companions) + shared mermaid SVG polish (`.mermaid` width/stroke/node radius/cluster fill) + quiet `<details>` styling. A site lists it first in `customCss` and MAY append its own bespoke CSS after. Centralizes the accent (and gives pm-skills, which sets none, the family accent) and the mermaid polish. Should also ship a slightly darker light-mode `--sl-color-text-accent`: `#5C7CFA` on white is 3.67:1, AA-borderline for body-size link text (no worse than Starlight's default, but fixable family-wide here). |
| `./public/favicon` (asset) | static image asset | The family default favicon, shipped so every site satisfies the 14.9 MUST without hand-rolling one (Starlight emits a `<link rel="icon">` on every page, so a missing favicon is a 404 on every page). Current placeholder: the `#5C7CFA` compass mark at [`assets/favicon.png`](assets/favicon.png); re-master as an SVG for crisp small-size rendering. |
...
| `./ci/ci-checks.mjs` (+ the four validators) | CI module | The portable guard suite (`node ci-checks.mjs all` / `dist` / `node-pin`) and the four build-aware validators (`check-rendered-links` with anchor resolution, `check-route-parity`, `verify-edit-links`, `remark-resolve-links`), parameterized by `base` and edit-base-URL. The reusable workflow's `build-cmd` invokes them. Version and parameterization live in one place; see [`ci-standard.md`](ci-standard.md). Extract from the FIXED agent-skills-toolkit versions, not the raw donor: they already carry the rollout-hardened fixes (bare-relative resolution, both quote styles, defensive path decode, the CLI `argv[1]` null-check). |
...
- The SEO surface the standard names: the family **favicon** (shipped as an asset so every site satisfies the 14.9 favicon MUST without hand-rolling one), a `robots.txt` **generated from the base** (removing the one sanctioned base literal each site otherwise carries in `public/robots.txt`), and a default `og:image`, so one bump reaches all sites.
